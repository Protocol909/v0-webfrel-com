"use client"

import type * as React from "react"
import { motion, useScroll, useTransform } from "framer-motion"
import { useRef } from "react"
import { cn } from "@/lib/utils"

export interface TextFadeProps {
  text: string
  className?: string
  wordClassName?: string
}

interface WordProps {
  children: string
  progress: any
  range: number[]
  className?: string
}

const Word: React.FC<WordProps> = ({ children, progress, range, className }) => {
  const opacity = useTransform(progress, range, [0, 1])

  return (
    <span className={cn("relative mr-1 text-xl md:text-2xl lg:text-3xl font-semibold", className)}>
      <span className="absolute opacity-20">{children}</span>
      <motion.span style={{ opacity: opacity }}>{children}</motion.span>
    </span>
  )
}

export const TextFade: React.FC<TextFadeProps> = ({ text, className, wordClassName }) => {
  const container = useRef(null)

  const { scrollYProgress } = useScroll({
    target: container,
    offset: ["start 0.9", "start 0.25"],
  })

  const words = text.split(" ")

  return (
    <p ref={container} className={cn("flex flex-wrap leading-relaxed", className)}>
      {words.map((word, i) => {
        const start = i / words.length
        const end = start + 1 / words.length

        return (
          <Word key={i} progress={scrollYProgress} range={[start, end]} className={wordClassName}>
            {word}
          </Word>
        )
      })}
    </p>
  )
}
